...
test("mobile navigation stays reachable and never covers scene content", async ({ page }) => {
  for (const viewport of [{ width: 320, height: 568 }, { width: 390, height: 844 }]) {
    await page.setViewportSize(viewport);
    for (const screen of ["S02", "S04", "S08", "S10", "S11", "S14"] as const) {
      ...
      const lastContent = await lastContentControl.count() ? lastContentControl : page.locator(`[data-scene="${screen}"]`).locator("p").last();
      const contentGeometry = await lastContent.evaluate((element) => {
        const rect = element.getBoundingClientRect();
