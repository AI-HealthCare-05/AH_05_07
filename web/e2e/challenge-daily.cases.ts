...
function windowWithActiveChallenge(options: { checkin?: "completed" | "skipped"; firstCheckin?: string | null; endsOn?: string } = {}) {
  const challenge = {
    id: "challenge-daily-active",
    action_id: "walk-10-minutes",
    starts_on: "2026-09-09",
    ends_on: options.endsOn ?? "2026-09-15",
    first_checkin_on: options.firstCheckin ?? "2026-09-09",
    status: "active",
  };
  return {
    start_on: "2026-09-05",
    ...
    challenge_events: [{ id: "challenge-daily-legacy", action_id: "sleep-routine", observed_on: "2026-09-08", status: "skipped" }],
  };
}
...
test("S03 saves an existing action request once without premature selection", async ({ page }) => {
  ...
  await expect(page.getByText("선택 요청을 저장하고 있어요.", { exact: false })).toBeVisible();
...
test("S07 labels the prior range as read-only instead of claiming today's state", async ({ page }) => {
  await setJourneyTime(page);
  ...
  await page.route("http://e2e.invalid/**", async (route) => {
    ...
    if (url.pathname.endsWith("/window")) return route.fulfill({ status: 200, headers, contentType: "application/json", body: JSON.stringify(windowWithActiveChallenge()) });
    return route.abort();
  });
