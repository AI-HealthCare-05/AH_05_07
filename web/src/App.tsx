...
function App() {
  ...
  function renderRecordLane(kind: RecordBrowseItem["kind"], title: string, emptyText: string, journal = false) {
  ...
  const journeyReview = import.meta.env.VITE_SK7_SCENE_MODE === "review";
  ...
  function renderScene() {
    ...
    if (activeScreen === "S02") {
      if (journeyReview) return <Scene id="S02" eyebrow="오늘" title={journeyCopy.S02.title} body="잠깐 머물러, 오늘을 남겨요." className="journey-candidate journey-today home-scene">
        <JourneyToday today={today} formattedDate={dateLabel(today)} lead={homeLead} secondary={homeSecondaryActions} measurementLabel={todayMeasurement ? "혈압 기록 있음" : "혈압 기록 전"} challengeLabel={activeChallenge && !activeChallengeEnded ? challengeLabel(activeChallenge.action_id) : "행동 선택 전"} onNavigate={navigate} />
      </Scene>;
    ...
    if (activeScreen === "S10") {
      if (journeyReview) return <Scene id="S10" eyebrow="기록의 발자국" title="7일 돌아보기" body="남겨둔 기록을, 천천히 돌아봐요." tone="water" className="journey-recap">
        <JourneyRecap today={today} year={startOn.slice(0, 4) === endOn.slice(0, 4) ? startOn.slice(0, 4) : `${startOn.slice(0, 4)}–${endOn.slice(0, 4)}`} prior={isPriorDashboard} freshness={windowState}
  ...
  return (
    <SceneShell activeScreen={activeScreen} evidenceLabel={fixture?.name} onNavigate={navigate} onSignOut={!evidenceMode ? () => void handleSignOut() : undefined} signOutPending={signOutPending || accountDeletionPending} companionSelection={companionSelection} savedSceneEvent={confirmedSave ? savedScene.event : null}>
