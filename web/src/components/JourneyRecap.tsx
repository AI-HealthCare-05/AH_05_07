    ...
    <section className="living-week recap-week-sheet" aria-labelledby="living-week-title" data-window-kind={period}>
      <div className="recap-week-intro">
        <div className="recap-week-overview">
          <header className="living-week-heading">
            <div>
              <p className="eyebrow">왜 7일을 모았을까요?</p>
              <h2 id="living-week-title">한 번의 숫자보다, 이어진 기록을 보기 위해서예요</h2>
              <p>{readOnly ? '지난 기록을 날짜 순서로 놓고, 무엇이 반복되고 달라졌는지 차분히 살펴봐요.' : '오늘까지의 기록을 날짜 순서로 놓고, 무엇이 반복되고 달라지는지 차분히 살펴봐요.'}</p>
            </div>
          </header>
      ...
      <div className="recap-day-focus" id="recap-day-context" data-day-focused={Boolean(selectedDay)}>
        {selectedDay ? <TrailDayDetail key={selectedDay.date} day={selectedDay} today={today} factsKnown={factsKnown}>
          ...
          : '혈압 관찰과 챌린지 참여를 날짜 순서로 따로 보면서, 한 번의 값만으로 결론내리지 않고 7일의 맥락을 확인해요. 이전 방식의 기록은 아래 목록에서 확인해요.'}</p>}
      </div>
    </section>
    ...
    <div className="recap-journal" data-main-section="seven-day-dashboard" data-focused-date={focusedDate ?? undefined}>
      ...
      <footer className="recap-tools">
        <p>{readOnly
          ...
          : '현재 7일의 기록을 파일로 보관해, 필요할 때 다시 확인하거나 의료진과 함께 볼 수 있어요.'}</p>
