...
해석을 따로 검증할 변화 추이 연구(C)를 구분한다. B/C는 현재 모델의 기능이 아니다.
현재 자료에 80+ 참여자가 포함되거나 한국어 handcard가 존재한다는 사실만으로
한국의80+ 사용자를 지원한다고 판단하지 않는다. 측정 BMI와 자기보고 숫자,
평생 경험과 현재 상태, 업무활동과 운동 일수의 입력 차이는
[adapter 인계 조건](model-input-adapter-contract.md#후속-구현-인계-조건)에서 계속 차단한다.
...
고도화 G3의 완료 범위는 목적·자료 역할·입력 구현 인계·평가 선행 조건을 기존
계약에 통합한 문서 설계다. 사람 결정 D1–D6는 미정이며 [결정 목록](model-release-readiness.md#사람에게-필요한-최소-결정-6개)에
담당 역할과 근거를 둔다. 문서·집계 JSON 검사는 실제 예측의 독립 재계산이나
모델 품질 검증을 추가한 것이 아니다. 기존 evidence·manifest·CONFIG·lock은
보존했고 실제 원자료/분할 파일은 접근하거나 해시하지 않았다.
