...
## S05 exception
Keep existing resolveProductionCompanion(mode, screen, confirmedSave) and existing boundary unchanged in the prototype.
Only confirmed persistence can produce celebrate exactly once then idle; failed/optimistic/unknown/conflict events cannot. Reduced motion skips action. No health meaning. Future migration requires per-confirmed-event deduplication across rerender/remount/back/forward/retry/tier changes. Do not replay missed celebration after visibility or motion preference changes. Current normal-lifecycle tests are not proof of every recovery path.
