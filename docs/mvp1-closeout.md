...
| NFR-01 | [부하 baseline](observation-load-baseline.md), Talos 5-1 | **검증 필요** | API별 부하·cold/warm·표본·실패율·P95 보고. 3초는 외부 평가 기준이며 기존 소표본 UI 중앙값은 그 증거가 아님 |
...
| 5-1 API P95 3초 | UI 수동 소표본 baseline만 있음 | API 전체 대상·부하·분위수 방식 사전 고정 후 별도 합성 부하 검증. 지금 새 품질 기준을 만들어 관측값을 통과시키지 않음 |
