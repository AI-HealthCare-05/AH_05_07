...
| FR-06 | P1 | Partial | User | Submit one of three fixed structured comprehension responses from S10 for review. | The first slice stores no free text, health values, record IDs, or Model V2 input/output; it allows at most one response per user/surface/Korea date, uses the 30-day owned-retention boundary, and is never an online-training label. Broader review/operator workflow remains outside this slice. |
