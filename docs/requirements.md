...
SK7 currently provides a seven-day record of home blood-pressure observations and one lifestyle challenge. Authenticated Model V2 production paths provide a versioned **입력 기반 위험군 선별 신호** through the frozen product contract. The API product projection remains `schema_version` plus `product_wording`. Signed-in S11 shows the browser-local continuous value as a transient **research/development preview** under the visibility, precision and expiry policy in the [Model V2 product contract](model-v2-product-contract.md#current-authority). It is not a probability, percentage, percentile, diagnosis, normal/abnormal result, risk band, severity, or future-incidence estimate, and it is not persisted. The legacy `/api/v1/risk-signal` scaffold remains separately `model_not_ready`. Model output, measured blood pressure, and challenge adherence remain separate facts. The service does not provide diagnosis, treatment, prevention, or causal-improvement claims.
...
| FR-01 | P0 | Implemented | User | Submit validated Model V2 product inputs for a versioned 입력 기반 위험군 선별 신호. | Reject missing, out-of-range, extra, or unit-ambiguous inputs. Signed-in S11 validates and computes the frozen path browser-locally without a feature-bearing inference POST; authenticated `/api/v1/model-v2/product-score` remains a separate server contract. Raw product input and inference result are not persisted. |
| FR-02 | P0 | Implemented | System | Keep the authenticated API projection at `schema_version` and `product_wording`; S11 shows the browser-local continuous output immediately after successful computation, without expanding details, within the [current product-contract window](model-v2-product-contract.md#current-authority). | The preview value is a plain decimal only: no probability/percentage/percentile conversion, threshold, band, diagnosis, normal/abnormal label, severity, future-incidence claim, persistence, telemetry, logging, S10 history, or PDF persistence. After the preview window, S11 returns to non-numeric output unless a new human decision explicitly extends it. If Model V2 is unavailable, keep the existing fail-closed behavior. |
...
| FR-06 | P1 | Partial | User | Submit one of three fixed structured comprehension responses from S10 for review. | The first slice stores no free text, health values, record IDs, or Model V2 input/output; it allows at most one response per user/surface/Korea date, uses the 30-day owned-retention boundary, and is never an online-training label. Internal review is limited to the existing Supabase project owner's non-persisted aggregate over the seven completed Korea dates; no reviewer API/UI or individual-row review path is added. Delegated review and external acceptance remain outside this slice. |
...
| NFR-02 | P0 | Planned | Equal normalized input plus model version returns equal output. | Repeated-input test against the immutable artifact. Historical [R1](research/model-v2-r1-artifact-result.md) and [R2](research/model-v2-r2-inference-result.md) evidence records byte-identical serialization, reload equivalence, and identical repeated inference for the frozen artifact. The current browser parity verifier covers product-adapter and runtime parity, but this change did not execute a current end-to-end repeated-input acceptance run with the external artifact; the status therefore remains Planned. |
...
- Frozen Model V2 input-based risk-group screening signal with the two-field API projection and the time-boxed S11 research/development preview defined by the Model V2 product contract; keep the legacy risk-signal scaffold separate and honestly unavailable
