...
S02 now registers one clay render and three viewport-specific posters per weekday. The poster includes the same bear and landmark, so fallback owns the whole decorative scene. Camera/source hashes and WebP bytes are pinned; a source or composition edit requires recapture before the normal build can pass. Posters use verified R2 public delivery with exact binary identity, WebP MIME, existing CORS and 4-hour cache evidence. Local originals remain available for reproducible authoring; new captures stay local until their public delivery is verified. See [clay poster workflow](scene-clay-posters.md).
F1 (SK7 3D First Paint F1) keeps the historical posters as canonical bytes, but identity-bound S02/S10 suppress them and use an identity-neutral CSS fallback until the exact selected realtime character is ready. F2 will recapture neutral posters; until then, no F1 runtime claims pixel-equivalent or recaptured poster output.
S10 uses the same calendar and landmark modules in a separately registered diorama: seven landmarks on desktop, the focal landmark and one neighbor on mobile. Every realtime recipe explicitly selects its root environment and registers all geometry dependencies. S10 has its own 21 matching posters; it cannot borrow S02 fallback art. See [S10 implementation and evidence](scene-s10-diorama.md).
...
- SceneRuntimeBoundary: stable scene + exact character identity boundary that owns the terminal failed latch for one route/identity visit. Motion-tier changes keep the same visit and its failure latch; only a route/identity unmount creates a new retry opportunity. SceneTierBoundary owns visibility, activation, timeout and the single poster fallback; the lazy error boundary reports failure without rendering a duplicate poster.
- First paint: one SceneShell-local active-visit channel accepts only the current token. A module counter supplies a new ephemeral token for each mounted realtime tier/recipe subvisit, including same-identity SPA returns. SceneRuntimeBoundary retains failure across those subvisits; readiness is local to the subvisit. Presence requires the active screen and exact descriptor URL/asset ID with ready phase; diagnostic DOM labels are not readiness authority. Real WebGL context loss can revoke readiness, and failure remains terminal until route/identity unmount.
- ThreeSceneRenderer: registered review geometry, camera and disposal only. A viewport profile change rebuilds S10 surroundings while retaining the character and renderer; route/recipe exit disposes assets and explicitly releases the WebGL context.
...
App owns one `useSeoulDate` snapshot for semantic dates, record-window bounds, S02 and S10. A timeout at the next Seoul midnight, visible-tab recovery and `pageshow` update it; fixed evidence `asOf` installs no clock. In-memory drafts stay mounted and retain their values, including a blood-pressure draft's selected date. Delayed callbacks read the committed presentation bounds and latest session; changed bounds invalidate earlier request generations before passive refresh effects. See [date rollover verification](scene-date-rollover.md). API payload dates still reflect the submitted input, not the completion time. Selecting a prior record window preserves the current calendar scenery; the existing loading placeholder can unmount/remount the decorative stage during that fetch.
