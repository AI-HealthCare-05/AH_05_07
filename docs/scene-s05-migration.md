...
The subsequent [Android device probe](scene-device-probe.md) measures ten S05 clips on physical Adreno hardware, verifies each new event and history-return non-repeat, checks zero idle RAF callbacks and context loss after exit, and samples retained JS heap after GC. [Device evidence](evidence/scene-android-performance.json) records the exact build. Its increasing heap/node observations require attribution; they do not establish a memory pass. Browser-emulated reduced motion is distinguished from OS settings, and callback intervals from displayed FPS.
