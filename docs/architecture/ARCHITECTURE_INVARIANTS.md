...
| REC-01 | Runtime rollback, schema reconstruction, and data recovery are distinct recovery operations and must not be represented as equivalent. | W recovery authority | `RECOVERY_CONTRACT.md` and `recovery-r10.json` | ENFORCED by R10 recovery contract. |
| REC-02 | Product retention, migration history, and repository state are not backups of production user rows or Auth identities. Unavailable recovery points remain explicit rather than inferred. | W recovery/data boundary | R10 recovery matrix and sanitized evidence | ENFORCED by R10 recovery contract; current provider-managed production data RPO is not established. |
| REC-03 | Isolated reconstruction must not use production credentials or production user/Auth/health data, and a successful isolated reconstruction is not evidence of a production managed restore. | W recovery verification | R10 isolated synthetic reconstruction and offline verifier | VERIFIED for the R10 drill. |
| REC-04 | R10 must not be marked complete while required production data RPO or database/Auth RTO remains an unresolved owner decision. Accepting residual risk does not make production data recoverable; setting a finite objective does not itself authorize backup/PITR/topology changes. | Owner decision; W recovery authority | `RECOVERY_CONTRACT.md` owner decision record | SATISFIED — owner accepted the current residual risk on 2026-09-09; no finite managed RPO/RTO was established and no backup/PITR/topology change was authorized. |
