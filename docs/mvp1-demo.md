...
Issue #225. **실제 제출 검토본 PPTX/PDF와 무음 자막 MP4를 제작·검사했다.**
[파일 목록·재생성·검증 범위](mvp1-submission-package.md)를 따른다. 최종 납품 수용과
사용자 검토는 미완료다. 아래는 제작 근거로 보존한 원고이며 영상의 실제 장면 순서는
현재/이전 구간·상세·빈 상태·오류/재시도·준비 중이다. 도면은 발표와 별도 SVG로 제공한다.
...
아래는 7장 제작 원고다. 실제 검토본은 발표자 노트와 동봉 출처 문서로 각 주장을 연결한다.
...
| 7 | 제출물7종·남은 결정·발주사 질의 | 실제 검토본 파일과 원본 위치, 후속 책임자 | 범위 수용·사용자 최종 검토 전 종료 선언 금지 |

발표 문장은 이번 제작에서 다듬었으며 제품 UI/질문 문구 개선은 후속이다. 승인된 JSON 수치를 발표 편의를 위해
