...
| Retention | Retention migrations are applied and history is known; RLS excludes every owned product row at `expires_at`; database triggers assign the 30-day deadline and reject extension; local exact-time retention pgTAP covers expired access, immutable expiry, and export exclusion; pg_cron purges daily. | O2 alternate evidence was accepted, but deployed expired-row invisibility before physical purge has not been verified with a synthetic expiry exercise. |
...
| R-03 | The 30-day lifecycle could be represented as purge-only rather than access-bound. | P0 | **Partial:** retention migrations are applied/history known, RLS is enabled, and local exact-time pgTAP covers `expires_at > now()` access denial, immutable expiry, and export exclusion; O2 alternate evidence was accepted. | Preserve the reviewed migration/RLS contract and retain daily jobs only for physical cleanup. Obtain separate production approval for a synthetic expiry exercise before claiming deployed closure. | The exact remaining gap is deployed expired-row invisibility before physical purge; closure requires approved deployed synthetic verification, not only migration history or local contract evidence. |
...
| R-06 | Error shapes differ between FastAPI validation and application errors. | P0 | **Partial:** `parseApiError()` maps FastAPI detail arrays to `validation_error`, object `detail.code/message` to a normalized `ApiRequestError`, unknown payloads to `request_failed`, and bounded timeout to `request_timeout`. `App.tsx` maps session 401, 422, 409/`observation_conflict`, `challenge_selection_locked`, load, export, delete, and save/update uncertainty to stable states. Browser evidence covers invalid input, expired session, load/unknown failure, storage failure, uncertain save/delete/update, timeout, stale refresh, and account boundaries; backend tests cover the stable conflict code. | Keep the source normalization boundary and current recovery states; add no implementation in this reconciliation. | **Remaining exact evidence gap:** no explicit browser assertion currently drives a server `409` / `observation_conflict` response through the web recovery mapping. |
