...
| Row ownership | Issue #149 Phase B passed anonymous denial, owner CRUD/export, cross-user non-disclosure, and active-challenge first-check-in lock with two synthetic accounts; cleanup completed. | Automated browser coverage remains separate. |
...
| Retention | Retention migrations are applied and history is known; RLS excludes every owned product row at `expires_at`; database triggers assign the 30-day deadline and reject extension; local exact-time retention pgTAP covers expired access, immutable expiry, and export exclusion; the owner-approved deployed synthetic expiry exercise verifies expired-row invisibility before physical purge; pg_cron purges daily. | Preserve the reviewed retention/RLS contract and repeat deployed synthetic evidence only after a contract-affecting retention/RLS change with separate approval. |
...
| R-03 | The 30-day lifecycle could be represented as purge-only rather than access-bound. | Resolved | Retention migrations/history are aligned; the RLS exact-time access boundary is present; local exact-time pgTAP covers expired access, immutable expiry, and export exclusion; owner-approved deployed synthetic verification confirms physical presence plus owner invisibility before purge, rollback, and residual rows 0. | Preserve the `expires_at > now()` RLS boundary, keep physical purge separate from access expiry, and repeat deployed synthetic evidence only after contract-affecting retention/RLS changes when separately approved. | [AC-05 deployed expiry evidence](evidence/ac05-deployed-expiry-invisibility.md) records the deployed migrations/history, exact-time RLS boundary, local pgTAP contract, owner-approved verification, physical presence, owner invisibility before purge, rollback, and zero residual synthetic Auth/BP rows. |
...
| R-06 | Error shapes differ between FastAPI validation and application errors. | Resolved | FastAPI validation normalization, application `detail.code` handling, unknown payloads to `request_failed`, bounded timeout to `request_timeout`, session `401` recovery, storage/request failures, stale refresh, uncertain mutation, and browser `409` / `observation_conflict` recovery are covered by source, API, and signed-in browser evidence. | Preserve the normalized error boundary and browser regression coverage. | Stable recovery states are covered, including an explicit browser assertion for server `409` / `observation_conflict`; this closes the R-06 evidence gap without changing source behavior. |
| R-07 | Operational logging and browser bundle boundary are documented but not fully audited. | Resolved boundary | The repository verifier checks application source and built browser assets for forbidden credential markers, JWT-like literals, private-key blocks, and request/BP-value logging. Issue #166 reviewed the bounded production Cloud Run requests stream after a synthetic refresh; only the absence result was retained. | Keep the verifier in CI and repeat the bounded review after a logging- or request-path change. | The point-in-time review found no secret, JWT, email or other identifier, request body, or health-value exposure; it makes no claim about future changes. |
