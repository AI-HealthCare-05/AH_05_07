...
| Row ownership | Issue #149 Phase A confirms the linked-project migration, table, RLS, policy-intent, and authenticated-grant metadata against the repository contract. | Metadata is not a two-user product-path proof; Phase B needs explicit owner approval. |
...
|---|---|---|---|---|---|
| R-01 | A legacy challenge-event record could be mistaken for the active seven-day challenge. | P0 | `challenge_events` remains only for the previous flow; new tables, API, and UI use `active_challenges` plus `challenge_checkins`; Issue #149 Phase A confirms linked metadata. | Keep the legacy route API-only, label it in the contract, and run the explicitly approved AC-06 synthetic-user checks before claiming closure. | Synthetic-user evidence proves one active challenge, seven-day check-ins, post-first-check-in lock, and no cross-user write. |
| R-02 | JWT/RLS ownership is configured but has no deployed two-user and anonymous negative test. | P0 | Issue #149 Phase A matches the linked metadata to the source contract; local pgTAP suites exist. | After explicit approval, use dedicated synthetic A/B accounts and anonymous access through the normal product boundary. | A cannot read/change B; anonymous cannot read/write; evidence is repeatable. |
