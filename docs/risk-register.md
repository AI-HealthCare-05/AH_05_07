...
| R-04 | Core record control is partial: BP edit/delete and recent-seven-day JSON export are available, but challenge-record edit and delete remain unavailable. | P0 | Requirements FR-07; current web supports BP ownership controls and a bounded own-record export. | Add owned challenge-record control flows with recovery states when the product contract is ready. | Browser flow and API tests verify the full owned-record lifecycle. |
...
| R-07 | Operational logging and browser bundle boundary are documented but not automatically audited. | P0 | No observation-body logging code was found in the current API; no automated secret/log scan proves it. | Add a redaction policy and CI/deployment checks for secret names, request body logging, and built client artifacts. | Review report shows no secret, JWT, or health-value exposure in source, bundle, and representative logs. |
| R-08 | A process outage can be confused with missing API runtime configuration. | P0 | `/live` and `/ready` have automated checks; production smoke evidence remains. | Keep liveness separate from configuration readiness and do not expose configuration values or product records. | Healthy and configuration-unready responses are independently verified in production. |
