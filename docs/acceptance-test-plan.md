...
| AC-05 | Synthetic users A and B plus an anonymous request attempt read, change, delete, and export. | Supabase integration | Only the owner can act on its records; cross-user access returns a non-disclosing result; anonymous requests fail. | Blocked: deployed RLS negative test absent. |
| AC-06 | User chooses one action, checks in, then attempts a second action and attempts to replace the first selection. | Database migration + API integration + browser | Exactly one active seven-day challenge; daily completed/skipped check-in works; replacement locks after first check-in. | Blocked: current model permits multiple actions. |
