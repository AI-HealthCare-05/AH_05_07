...
def inspect_glb(data):
    ...
    clips = inspect_clips(document, binary)
    triangles = 0
    for primitive in primitives:
        ...
        accessor = (
            ...
            else accessors[primitive["attributes"]["POSITION"]]
        )
        triangles += accessor["count"] // 3
    return {
        "mesh_count": len(document.get("meshes", [])),
        "primitive_count": len(primitives),
        "unique_geometry_triangles": triangles,
...
def inspect_clips(document, binary):
    accessors = document.get("accessors", [])
    clips = []
    for animation in document.get("animations", []):
        ...
        for sampler in animation["samplers"]:
            ...
            if "max" in accessor:
                ...
                durations.extend(
                    struct.unpack_from("<f", binary, start + i * stride)[0] for i in range(accessor["count"])
                )
        clips.append(
            {
                "name": animation.get("name"),
                "duration_seconds": max(durations, default=None),
                "channels": len(animation["channels"]),
                "translation_targets": sorted(
                    {c["target"]["node"] for c in animation["channels"] if c["target"]["path"] == "translation"}
                ),
            }
        )
    return clips
...
def main():
    ...
    report = {
        "schema_version": 1,
        "source_commit": "9361983ea00afa244da560ac62f0d533e9f9e942",
        "status": "HASH_VERIFIED_STRUCTURE_INSPECTED",
        "asset_count": len(results),
        "unique_species_clip_count": len(unique_clips),
        "binary_mutations": 0,
        "runtime_performance": "NOT_MEASURED",
        "assets": results,
    }
