...
def main():
    ...
    try:
        ...
        lock_digest = historical_toolchain_lock_sha256(gate, ROOT)
        if evidence_findings(gate) or repository_alignment_findings(gate, ROOT) or lock_digest is None:
            raise ValueError("gate_mismatch")
        validate_evidence(json.loads(args.evidence.read_text(encoding="utf-8")), gate, load_manifest(), lock_digest)
    except Exception:
