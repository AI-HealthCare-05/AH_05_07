...
def subset(data, names):
    ...
    for old, new in accessor_map.items():
        ...
        assert before == after
        assert (
            binary[bv.get("byteOffset", 0) : bv.get("byteOffset", 0) + bv["byteLength"]]
            == packed[av["byteOffset"] : av["byteOffset"] + av["byteLength"]]
        )
...
def main():
    ...
    report = {
        "status": "OFFLINE_EXPERIMENT_NOT_FOR_RUNTIME",
        "source_sha256": source_hash,
        "derivative_sha256": hashlib.sha256(result).hexdigest(),
        "source_bytes": len(data),
        "derivative_bytes": len(result),
        "saved_bytes": len(data) - len(result),
        "retained_clips": ["idle", "rest", "celebrate"],
        "retained_accessor_count": count,
        "retained_accessor_bytes_and_metadata_identical": True,
        ...
        "visual_parity": "NOT_MEASURED",
        "runtime_decode": "NOT_MEASURED",
        "immutable_sources_modified": 0,
