...
| 선택형 생활정보 도구 | 원할 때 생활정보를 동결된 11개 의미 특성 계약에 맞춰 Model V2 경로에서 처리하고, 입력한 활동·수면·생활습관을 이번 이용에만 보이는 `오늘의 시작점` 요약으로 정리합니다. 개인별 점수·확률·백분율·등급은 표시하지 않고, 입력과 내부 결과도 저장하지 않습니다. |
