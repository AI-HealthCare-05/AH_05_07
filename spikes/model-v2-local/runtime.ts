...
const unavailable = () => new Error("inference_unavailable");
...
export async function loadVerifiedModel(expectedHash: string): Promise<Model> {
...
// Internal values are used by the isolated verification entry only. The demo
// calls scoreProduct and exposes only the existing two-field projection.
...
export function scoreProduct(model: Model, input: unknown): Projection {
  evaluateSemantic(model, adaptProductInput(input));
