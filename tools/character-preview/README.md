...
현재 카탈로그에는 **11종·77개 고유 동작**이 선언한 로컬 기술 QA 범위의 통과로
선정되어 있다. 사용자는 2026-09-06 현재 완료분까지만 제작을 마감하도록 범위를
조정했다. 추가 물범 제작은 이번 작업에서 제외하며 사람 디자인 승인은 대기다.
기존 곰 v007·토끼 v002·강아지 v001·고양이 v002·레서판다 v003·
수달 v003·카피바라 v002의 결과를 보존하고, 펭귄 v002·다람쥐 v001·고슴도치 v004·
여우 v002를 별도 실행·시각 검토 후 추가했다. 표준/경량을 고유 동작 수에 중복 합산하지 않는다.
새 네 후보의 실제 viewer는 `b28596c3e2ac8390364621c49d38a3983ea497ec`이며,
후보별 생성 SHA와 이번 문서 반영 커밋을 구분한다. 원본 열기·전체 loop·고정 바닥·
영상·추가 표적 검토 및 외부 근거는 [현재 확인된 범위](../../docs/character-preview-verification.md#현재-확인된-범위)에,
기존 세 후보의 이력은 [레서판다·수달·카피바라 실행 기록](../../docs/character-preview-verification.md#새-세-후보-실행-기록과-한계)에 있다.

물범은 박수 미접촉과 제한된 구조안의 몸통/지느러미 교차로 `needs_revision`이며
카탈로그의 GLB/hero 경로를 비워 선택 수량에서 제외했다. 실패 원본을 보존하며 같은
구조안 계열의 추가 탐색은 중단했다. 모든 후보의 사람 디자인 승인은 대기다.
물리 접지·전 프레임 충돌·실제 모바일/운영 성능·제품 출시는 승인하지 않는다.
작은 꼬리 면의 기존 normal 진단, 새 후보의 표본 검토 한계와 진단 영상의 스크롤/
회색 여백도 기록한다. 22개 보관용 WebM은 검증 원본과 동일한 바이트의 로컬 사본이다.
