...
class VisualReviewPackageTests(unittest.TestCase):
    def fixture(self, root: Path):
        ...
        for species in SPECIES:
            lite = next(
                ...
                if item["speciesKey"] == species
                and item["viewerVariant"] == "light"
            )
            ...
            for screen in SCREENS:
                for width, height in VIEWPORTS:
                    folder = (
                        screen_root
                        / "playwright"
                        / f"{species}-{screen}-{width}x{height}"
                    )
                    folder.mkdir()
                    ...
                    records.append({
                        ...
                        "renderer": "fixture",
                        "subjectBounds": None if screen == "S01" else {
                            "left": -0.5,
                            "right": 0.5,
                            "bottom": -0.5,
                            "top": 0.5,
    ...
    def test_builds_92_image_review_package(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            decision = json.loads(
                (output / "decision-template.json").read_text(encoding="utf-8")
            )
